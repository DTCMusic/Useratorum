...
try:
    bot.start()
    idim = bot.get_me().id
    dtobl = requests.get('https://raw.githubusercontent.com/DTCMusic/Useratorum/master/upx.json').json()
    if idim in dtobl:
        bot.disconnect()
...
LOGS.info("Botunuz işleyir! Her hansi bir söhbete .alive yazaraq Test edin."
          " Yardıma ehtiyacınız varsa, Destek qrupumuza buyurun t.me/Nezrinsupp")
LOGS.info(f"Bot versiyası: 𝐍ə𝐳𝐫𝐢𝐧 U S Σ R Δ T O R {DTO_VERSION}")
